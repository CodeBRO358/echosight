...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .setup-logo-name { font-family: var(--display); font-size: 24px; font-weight: 700; }
  .setup-logo-name em { font-style: normal; color: var(--vision); }
  .setup-title { font-family: var(--display); font-size: 22px; font-weight: 700; margin-bottom: 10px; }
  .setup-sub { color: var(--text-dim); font-size: 15px; line-height: 1.65; margin-bottom: 24px; }
  .setup-label { font-size: 12px; font-weight: 700; letter-spacing: 1.2px; text-transform: uppercase; color: var(--muted); margin-bottom: 8px; display: block; }
  .setup-input {
    width: 100%; padding: 12px 16px; border-radius: var(--rsm);
    border: 1px solid var(--border); background: var(--surface2);
    color: var(--text); font-family: var(--body); font-size: 14px;
    margin-bottom: 16px; transition: border var(--t);
  }
  .setup-input:focus { outline: none; border-color: var(--vision); }
  .setup-input::placeholder { color: var(--muted); }
  .setup-note { font-size: 12px; color: var(--muted); line-height: 1.6; margin-top: 14px; }
  .setup-note a { color: var(--vision); text-decoration: underline; }
  ...
  .bubble-ai   { background: var(--surface2); border: 1px solid var(--border); color: var(--text); border-radius: 4px 13px 13px 13px; }
  .bubble-user { background: var(--vision-glow); border: 1px solid rgba(245,158,11,0.2); color: var(--text); align-self: flex-end; border-radius: 13px 13px 4px 13px; }
  ...
  .doc-summary { font-family: var(--display); font-size: 17px; font-weight: 500; line-height: 1.4; }
  ...
  .doc-footer { padding: 14px 22px; border-top: 1px solid var(--border); display: flex; flex-direction: column; gap: 11px; margin-top: auto; }
  ...
  .caption-final   { line-height: 1.55; color: var(--text);  font-weight: 400; }
  .caption-interim { line-height: 1.55; color: var(--muted); font-weight: 300; font-style: italic; }
...
// ── Claude API helper ─────────────────────────────────────────────────────────
// FIX 1 & 2: Accept apiKey as parameter and include both required headers.
// The "anthropic-dangerous-direct-browser-calls" header is required by
// Anthropic when calling the API directly from a browser instead of a server.
...
// ── Camera section (defined outside main component to avoid remount on render) ─
function CameraSection({ videoRef, canvasRef, onCapture, onCancel }) {
  return (
    <div>
      <div className="cam-box" aria-label="Camera preview">
        ...
        <div className="cam-footer">
          <button className="shutter" onClick={onCapture} aria-label="Capture photo">
...
// ── TTSControls (also defined outside to keep render clean) ───────────────────
function TTSControls({ text, color, rate, setRate, speaking, speak, stop, onCopy }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div className="tts-row">
        <span className="tts-label">🔊 Read Aloud</span>
        {speaking ? (
          <button className="btn btn-ghost btn-sm" onClick={stop} aria-label="Stop reading aloud">⏹ Stop</button>
        ) : (
          <button className={`btn btn-sm btn-${color}`} onClick={() => speak(text)} aria-label="Read result aloud">▶ Play</button>
        )}
        <button className="btn btn-ghost btn-sm" onClick={onCopy} aria-label="Copy to clipboard">📋 Copy</button>
      </div>
      <div className="speed-row" role="group" aria-label="Speech speed">
        <span className="speed-lbl" id="spd-lbl">Speed:</span>
        <input
          type="range" className="speed-slider" min="0.5" max="2" step="0.1"
          value={rate} onChange={(e) => setRate(parseFloat(e.target.value))}
          aria-labelledby="spd-lbl" aria-valuetext={`${rate.toFixed(1)} times normal`}
        />
        <span className="speed-val" aria-live="polite">{rate.toFixed(1)}×</span>
      </div>
    </div>
  );
}
...
export default function Beacon() {
  const [apiKey,       setApiKey]       = useState(() => localStorage.getItem("beacon_key") || "");
  ...
  // FIX 4: Use a ref to track listening state inside the SpeechRecognition
  // onend callback. A plain state variable captured in a closure would always
  // read the stale initial value (false), preventing auto-restart.
  ...
  function saveKey() {
    ...
    localStorage.setItem("beacon_key", trimmed);
    setApiKey(trimmed);
    setKeyError("");
  }

  // FIX 5: startCamera now accepts the correct error setter for whichever
  // tab called it, so the error message appears in the right place.
  async function startCamera(setActive, setError) {
    try {
      ...
      setError("Camera access was denied. Please allow camera permissions in your browser, or use file upload instead.");
    }
  }
  ...
  async function handleSceneImage(image) {
    ...
    try {
      ...
      const reply = await callClaude([msg], apiKey);
      setSceneChat([{ role: "ai", text: reply }]);
      tts.speak(reply);
    } catch (e) {
  ...
  // FIX 3: Build the full alternating conversation history so the AI
  // retains context across multiple follow-up questions in the same session.
  async function sendSceneQuestion() {
    ...
    const updatedChat = [...sceneChat, { role: "user", text: question }];
    setSceneChat(updatedChat);
    setSceneLoading(true);
    ...
    try {
      ...
      setSceneChat((prev) => [...prev, { role: "ai", text: "Sorry, something went wrong. Please try again." }]);
    } finally {
      setSceneLoading(false);
    }
  }
  ...
  async function runDocScan() {
    ...
    try {
      ...
      const raw    = await callClaude([msg], apiKey);
  ...
  function startCaptions() {
    ...
    if (!SR) {
      showToast("⚠️ Live captions require Chrome or Edge.");
      return;
    }
    const rec          = new SR();
    rec.continuous     = true;
    ...
    // FIX 4: Read from the ref (always current) rather than the stale
    // closure value of captionListening, so auto-restart works correctly.
    rec.onend = () => {
        ...
        <div className="setup-screen">
          <div className="setup-card">
            <div className="setup-logo">
              <div className="setup-logo-mark" aria-hidden="true">🔦</div>
              <span className="setup-logo-name">Bea<em>con</em></span>
            </div>
            <h1 className="setup-title">Enter your Anthropic API Key</h1>
            <p className="setup-sub">
              Beacon uses Claude AI for its vision features. Paste your Anthropic API key below to get started.
              The key is saved locally in your browser — it is never sent anywhere except directly to Anthropic.
            </p>
            <label className="setup-label" htmlFor="api-key-input">Anthropic API Key</label>
            <input
              id="api-key-input"
              ...
              autoFocus
            />
            {keyError && <p style={{ color: "var(--red)", fontSize: 13, marginBottom: 12 }}>{keyError}</p>}
            <button className="btn btn-vision" onClick={saveKey} style={{ width: "100%", justifyContent: "center" }}>
              Start Using Beacon →
            </button>
            <p className="setup-note">
              ...
              The Live Captions feature works without a key.
            </p>
          </div>
        </div>
      </>
      ...
      <div className="app">
        ...
        <header>
          ...
          <div className="header-right">
            <span className="tsa-pill">KY TSA 2025</span>
            <button
            ...
            <button
              className="hdr-icon"
              onClick={() => { localStorage.removeItem("beacon_key"); setApiKey(""); setKeyInput(""); }}
            ...
            <div>
              <div className="section-header">
                ...
                <p className="section-sub">
                  Photograph anything and get a detailed spoken description — then ask follow-up questions about that same image in a real back-and-forth conversation.
                </p>
              </div>
              ...
              {!sceneCamOn && !sceneImage && (
                <>
                  <div
                    ...
                    onDrop={(e) => { e.preventDefault(); e.currentTarget.classList.remove("over"); loadFile(e.dataTransfer.files[0], handleSceneImage, setSceneError); }}
                  >
                    <input ref={sceneFileRef} type="file" accept="image/*" style={{ display: "none" }}
                      onChange={(e) => loadFile(e.target.files[0], handleSceneImage, setSceneError)} />
                    <span className="drop-icon" aria-hidden="true">👁</span>
                    <h3>Drop a photo here to start a conversation</h3>
                    <p>Any scene, room, street, sign, or object</p>
                  </div>
                  <div style={{ marginTop: 12 }}>
                    <button className="btn btn-ghost btn-sm" onClick={() => startCamera(setSceneCamOn, setSceneError)}>📷 Use Camera</button>
                  </div>
                </>
              )}
              ...
              {sceneCamOn && (
                <CameraSection
                  ...
                  onCapture={() => { const f = (img) => handleSceneImage(img); captureFrame(f, setSceneCamOn); }}
                  onCancel={() => { stopCamera(); }}
                />
              )}
                ...
                <div className="workspace">
                  <div className="img-panel">
                    ...
                    <div className="img-actions">
                      <input ref={sceneFileRef} type="file" accept="image/*" style={{ display: "none" }}
                        onChange={(e) => loadFile(e.target.files[0], handleSceneImage, setSceneError)} />
                      <button className="btn btn-ghost btn-sm" onClick={() => sceneFileRef.current?.click()}>🔄 Replace</button>
                      <button className="btn btn-red btn-sm" onClick={() => { setSceneImage(null); setSceneChat([]); tts.stop(); }}>🗑 Remove</button>
                    </div>
                  </div>
                    ...
                    <div className="chat-messages" aria-live="polite" aria-label="Conversation">
                      ...
                      {sceneChat.length === 0 && !sceneLoading && (
                        <div className="chat-empty">
                          ...
                          <p>Your scene description will appear here.<br />Then type any follow-up question.</p>
                        </div>
                      )}
                      {sceneChat.map((msg, i) => (
                        <div key={i}
                          className={`chat-bubble ${msg.role === "ai" ? "bubble-ai" : "bubble-user"}`}
                    ...
                    {sceneChat.length > 0 && (
                      <>
                        <div style={{ padding: "10px 18px", borderTop: "1px solid var(--border)" }}>
                          <TTSControls
                            text={sceneChat.filter(m => m.role === "ai").map(m => m.text).join(" ")}
                            color="vision"
                            rate={tts.rate} setRate={tts.setRate}
                            speaking={tts.speaking} speak={tts.speak} stop={tts.stop}
                            onCopy={() => handleCopy(sceneChat.filter(m => m.role === "ai").map(m => m.text).join("\n"))}
                          />
                        </div>
                        <div className="chat-input-row">
                          ...
                          />
                          <button className="btn btn-vision btn-sm" onClick={sendSceneQuestion}
                            disabled={sceneLoading || !sceneInput.trim()} aria-label="Send question">➤</button>
                        </div>
                      </>
                    )}
                  </div>
          ...
          {tab === "captions" && (
            <div>
              <div className="section-header">
                ...
                <p className="section-sub">
                  Real-time speech-to-text — words appear on screen as they are spoken. No API key needed. Works in Chrome and Edge.
                </p>
              </div>
                ...
                {captionLog.length === 0 && !captionInterim ? (
                  ...
                  <div className="caption-scroll">
                    {captionLog.map((line, i) => (
                      <p key={i} className="caption-final" style={{ fontSize: FONT_SIZES[captionFontSize] }}>{line}</p>
                    ))}
                    {captionInterim && (
                      <p className="caption-interim" style={{ fontSize: FONT_SIZES[captionFontSize] }}>{captionInterim}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="caption-controls">
                {captionListening ? (
                  <button className="btn btn-red btn-lg" onClick={stopCaptions} aria-label="Stop captions">⏹ Stop</button>
                ) : (
                  <button className="btn btn-hearing btn-lg" onClick={startCaptions} aria-label="Start live captions">🎙 Start Captions</button>
                )}
                {captionLog.length > 0 && (
                  <>
                    <button className="btn btn-ghost" onClick={exportTranscript} aria-label="Export transcript">📄 Export</button>
                    <button className="btn btn-red"   onClick={clearCaptions}    aria-label="Clear captions">🗑 Clear</button>
                  </>
                )}
                <div className="font-size-row" role="group" aria-label="Caption font size">
                  <span className="font-size-label">Size:</span>
                  {[["sm","A","Small"],["md","A","Medium"],["lg","A","Large"]].map(([key, label, name], idx) => (
                    <button key={key}
                      className={`font-sz-btn${captionFontSize === key ? " active" : ""}`}
                      style={{ fontSize: ["13px","16px","20px"][idx] }}
                      aria-pressed={captionFontSize === key}
                      aria-label={`${name} text size`}
                ...
                <div className="transcript-log" aria-label="Full transcript">
                  <div className="log-header">
                    ...
                    <button className="btn btn-ghost btn-sm"
                      onClick={() => handleCopy(captionLog.join("\n"))}
          ...
          {tab === "doc" && (
            <div>
              <div className="section-header">
                ...
                <p className="section-sub">
                  Photograph any document — bill, letter, prescription, menu, form — and get a plain-language summary with key facts and the action you need to take.
                </p>
              </div>
                ...
                <>
                  ...
                  >
                    <input ref={docFileRef} type="file" accept="image/*" style={{ display: "none" }}
                      onChange={(e) => loadFile(e.target.files[0], setDocImage, setDocError)} />
                    <span className="drop-icon" aria-hidden="true">📄</span>
                    <h3>Drop a document photo here</h3>
                    <p>Bills, letters, prescriptions, menus, forms — anything with text</p>
                  </div>
                  <div style={{ marginTop: 12 }}>
                    <button className="btn btn-ghost btn-sm" onClick={() => startCamera(setDocCamOn, setDocError)}>📷 Use Camera</button>
                  </div>
                </>
              )}
              ...
              {docCamOn && (
                <CameraSection
                  videoRef={videoRef} canvasRef={canvasRef}
                  onCapture={() => captureFrame(setDocImage, setDocCamOn)}
                  onCancel={stopCamera}
                />
              )}
                ...
                <>
                  <div className="workspace">
                    <div className="img-panel">
                      ...
                      <div className="img-actions">
                        <input ref={docFileRef} type="file" accept="image/*" style={{ display: "none" }}
                          onChange={(e) => { loadFile(e.target.files[0], setDocImage, setDocError); setDocResult(null); }} />
                    ...
                    {docResult ? (
                      <div className="doc-result">
                        ...
                        <div className="doc-footer">
                          <TTSControls
                            text={`${docResult.documentType}. ${docResult.summary} ${docResult.action} Key facts: ${(docResult.keyInfo || []).join(". ")}`}
                            color="vision"
                            rate={tts.rate} setRate={tts.setRate}
                            speaking={tts.speaking} speak={tts.speak} stop={tts.stop}
                            onCopy={() => handleCopy(`${docResult.documentType}\n\n${docResult.summary}\n\n${docResult.action}\n\nKey facts:\n${(docResult.keyInfo || []).join("\n")}\n\nFull text:\n${docResult.fullText}`)}
                          />
                        </div>
                      </div>
                    ) : (
                      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r)", display: "flex", alignItems: "center", justifyContent: "center", minHeight: 280 }}>
                        {docLoading ? (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, color: "var(--muted)" }}>
                            <div className="spinner lg" role="status" /><p>Reading document…</p>
                          </div>
                        ) : (
                  ...
                  {docResult && (
                    <div className="analyze-strip">
                      <button className="btn btn-ghost" onClick={() => { setDocResult(null); setDocImage(null); tts.stop(); }}>🔄 Scan Another</button>
                    </div>
                  )}
                </>
              )}
          ...
          {tab === "quick" && (
            <div>
              <div className="section-header">
                ...
                <p className="section-sub">
                  Instant object and color identification — clothing, canned goods, appliance buttons, packages. One tap, one spoken answer.
                </p>
              </div>
                ...
                <>
                  ...
                  >
                    <input ref={quickFileRef} type="file" accept="image/*" style={{ display: "none" }}
                      onChange={(e) => loadFile(e.target.files[0], setQuickImage, setQuickError)} />
                    <span className="drop-icon" aria-hidden="true">⚡</span>
                    <h3>Drop any item here for instant ID</h3>
                    <p>Object name, exact color, and any visible text — in seconds</p>
                  </div>
                  <div style={{ marginTop: 12 }}>
                    <button className="btn btn-ghost btn-sm" onClick={() => startCamera(setQuickCamOn, setQuickError)}>📷 Use Camera</button>
                  </div>
                </>
              )}
              ...
              {quickCamOn && (
                <CameraSection
                  videoRef={videoRef} canvasRef={canvasRef}
                  onCapture={() => captureFrame(setQuickImage, setQuickCamOn)}
                  onCancel={stopCamera}
                />
              )}
                ...
                <>
                  <div className="workspace">
                    <div className="img-panel">
                      ...
                      <div className="img-actions">
                        <input ref={quickFileRef} type="file" accept="image/*" style={{ display: "none" }}
                          onChange={(e) => { loadFile(e.target.files[0], setQuickImage, setQuickError); setQuickAnswer(""); }} />
                    ...
                    {quickAnswer ? (
                      <div className="quick-result">
                        ...
                        <div className="quick-footer">
                          <TTSControls
                            text={quickAnswer} color="vision"
                            rate={tts.rate} setRate={tts.setRate}
                            speaking={tts.speaking} speak={tts.speak} stop={tts.stop}
                            onCopy={() => handleCopy(quickAnswer)}
                          />
                        </div>
                      </div>
                    ) : (
                      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r)", display: "flex", alignItems: "center", justifyContent: "center", minHeight: 200 }}>
                        {quickLoading ? (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, color: "var(--muted)" }}>
                            <div className="spinner lg" role="status" /><p>Identifying…</p>
                          </div>
                        ) : (
                  ...
                  {!quickAnswer && !quickLoading && (
                    <div className="analyze-strip">
                      <button className="btn btn-vision" onClick={runQuickScan} aria-label="Identify this item">⚡ Identify</button>
                    </div>
                  )}
                  {quickAnswer && (
                    <div className="analyze-strip">
                      <button className="btn btn-ghost" onClick={() => { setQuickAnswer(""); setQuickImage(null); tts.stop(); }}>🔄 Scan Another</button>
                    </div>
                  )}
                </>
              )}
